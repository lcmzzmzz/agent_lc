...
class EcommerceEvalRequest(BaseModel):
    """POST /api/ecommerce/eval/run 请求体。"""
...
async def ecommerce_eval_run(req: EcommerceEvalRequest) -> dict:
    """批量跑金色用例评估，写出 summary/cases/case-index 到 output_dir。"""
    logger.info(
        f"[ecommerce] POST eval run cases_path='{req.cases_path}' output_dir='{req.output_dir}'"
    )
...
async def ecommerce_eval_result(
    eval_run_id: str,
    # 【正经注释】FIX-6：output_dir 必须是查询参数（默认与 POST 的 EcommerceEvalRequest.output_dir 一致），
    # 不能硬编码 outputs/ecommerce/eval-runs —— 否则自定义 output_dir 的评估跑出来的结果读不回来。
    # 【大白话注释】GET 时也得能告诉服务器"我那次跑存在哪"，否则只能找到默认目录里的，自定义目录全 404。
    output_dir: str = "outputs/ecommerce/eval-runs",
) -> dict:
    """按 eval_run_id 反查本次评估的 summary.json。

    【正经注释】FIX-4：找不到文件必须返回 404，而不是让 FastAPI 把 FileNotFoundError 当 500 泄露。
    """
