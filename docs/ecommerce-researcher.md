...
ProductResearchPlannerAgent          # 拆解为趋势/竞品/评论/风险查询
  │
  ▼  并发执行（LangGraph StateGraph fork-join）
...
| `graph.py` | LangGraph StateGraph fork-join 编排（planner→三路并发→scoring），Annotated reducer 合并分支 |
| `runner.py` | 端到端入口 + 文件输出 + 默认检索器适配 |
| `schemas/` | 评分加权、报告章节结构 |

### 双数据源架构（Tavily 广域检索 + Apify 平台评论）

评论洞察走可插拔的双层数据源（`multi_agents/ecommerce/tools/review_scraper.py`）：

| 层 | 数据源 | 负责 | 适用 Agent |
|----|--------|------|-----------|
| 广域检索 | **Tavily**（默认检索器） | 趋势 / 竞品 / 评测 / 风险（开放网页） | Trend / Competitor / 报告引用 |
| 平台评论 | **Apify**（可插拔 Scraper） | Amazon / Reddit **真实评论**（rating / date / helpful / url） | ReviewInsightAgent |

- 有 `APIFY_API_TOKEN` → `ApifyReviewScraper` 抓真实评论（结构化 `ReviewItem`）
- 无 token 或 Apify 运行时失败 → 自动降级 `FallbackSearchReviewScraper`（Tavily 摘要兜底），**demo 照跑**
- 接入指南：`docs/ecommerce-apify-setup.md`
- 可观测：audit 记录 `review_source`(apify/web_fallback) / `review_count` / `platforms` / `fallback_reason`，evaluation 也含 `review_source/review_count`
- 扩展：新平台（TikTok/YouTube/GMaps）只需实现一个 `ReviewSource` 加进 `DEFAULT_ACTORS`
