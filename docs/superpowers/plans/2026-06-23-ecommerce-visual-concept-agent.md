# Ecommerce Visual Concept Agent Implementation Plan
...
- `doubao-seedream-4-5-251128` must not send `output_format`.
- `doubao-seedream-5-0-260128` may send `output_format="png"` only through explicit model capability logic.
...
```bash
py -m pytest tests/test_ecommerce_state.py::test_create_initial_state_contains_visual_result tests/test_ecommerce_evaluation.py::test_build_evaluation_summary_counts_visual_metrics -q
```
...
```bash
py -m pytest tests/test_ecommerce_state.py tests/test_ecommerce_evaluation.py -q
```
...
@pytest.mark.asyncio
async def test_seedream_45_does_not_send_output_format(tmp_path):
...
```bash
py -m pytest tests/test_ecommerce_image_provider.py -q
```
...
- [ ] **Step 4: Implement VolcArkJimengProvider**
...
def _supports_output_format(model: str) -> bool:
    return model.startswith("doubao-seedream-5-")
...
```bash
py -m pytest tests/test_ecommerce_visual_agent.py -q
```
...
async def test_runner_writes_visual_artifacts_when_enabled(tmp_path):
    ...
    assert result["output_paths"]["visual_assets"].endswith("portable-blender-visual/visual-assets.json")
...
Add this test to `tests/test_ecommerce_human_review.py`:
...
If `tests/test_ecommerce_human_review.py` does not import `json`, add:
...
```bash
py -m pytest tests/test_ecommerce_runner_visual.py tests/test_ecommerce_human_review.py::test_load_run_reads_visual_result -q
```

Expected: FAIL because runner does not accept visual args and run store does not load visual artifacts.

- [ ] **Step 4: Modify runner signature and visual stage**
...
After graph completion and before `evaluation_summary = build_evaluation_summary(final_state)`, compute output paths and run visual generation:
...
Remove the later duplicate `output_path = Path(output_dir)` and `slug = slugify(query)` declarations so these variables are only defined once.

When building `final_state["output_paths"]`, add visual paths only when enabled:
...
```bash
py -m pytest tests/test_ecommerce_runner_visual.py tests/test_ecommerce_human_review.py -q
```
...
```bash
py -m pytest tests/test_ecommerce_api.py::test_ecommerce_research_passes_visual_fields tests/test_ecommerce_api.py::test_ecommerce_websocket_passes_visual_fields -q
```
...
```bash
py -m pytest tests/test_ecommerce_api.py -q
```
...
```bash
py -m pytest tests/test_ecommerce_frontend_visual.py -q
```
...
function renderVisualResult(visual){
  ...
  $("visualGrid").innerHTML = rows.map(row => {
    ...
    const image = imageUrl ? `<img src="${escWarn(imageUrl)}" alt="${escWarn(row.asset_id || "visual asset")}" />` : "";
    return `
      <div class="visual-item">
        ${image}
        <div class="visual-meta">
          <div><span class="pill">${escWarn(row.kind || "-")}</span> <span class="pill">${escWarn(row.slot || "-")}</span></div>
          <div>Status: ${escWarn(row.status || "-")}</div>
          <div>${escWarn(row.reason || "")}</div>
          <details><summary>Prompt</summary><div>${escWarn(row.prompt || "")}</div></details>
          ${row.warning ? `<div class="warn">${escWarn(row.warning)}</div>` : ""}
        </div>
      </div>`;
  }).join("");
}
...
```bash
py -m py_compile scripts/smoke_jimeng_image_generation.py
```
...
```bash
set ARK_API_KEY=<your-rotated-key>
...
- Safety:
  ...
  - `doubao-seedream-4-5-251128` does not send `output_format`.
