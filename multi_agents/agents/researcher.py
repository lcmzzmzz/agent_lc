...
class ResearchAgent:
    ...
    async def run_initial_research(self, research_state: dict):
        ...
        context = await self.research(query=query, verbose=task.get("verbose"),
                                      source=source, tone=self.tone, headers=self.headers)
        print(f'browser环节：  {context}')
        return {"task": task, "initial_research": context }
    ...
    async def run_depth_research(self, draft_state: dict):
        ...
        print(f'researcher环节中的researcher子环节：  {research_draft}')
